...
let Assistant = {
  createNew: recognition_result => {
    ...
    // Step1. 尋找句子的動詞
    for (let whichword = 0; whichword <= (InputText.length + 2); whichword++) {
      let word = InputText.slice(whichword, whichword + 2);
      // console.log(word); 測試用
      //建立投票
      if (word == "建立" || word == "開設" || word == "設立" || word == "創建" || word == "新增" || word == "增加") {
        text_flag = true;
        text_site = whichword;
        text_type = 1;
        console.log('Step1-1_建立');
        break;
      } else if (word == "開始" || word == "使用" || word == "開啟" || word == "玩") {
        text_flag = true;
        text_site = whichword;
        text_type = 2;
    ...
    function AddEvent(text_site) {
      for (let whichword = text_site; whichword <= (text_site + 4); whichword++) {
        let word = InputText.slice(whichword, whichword + 2);
        if (word == "投票") {
          //開啟Toolbar.js的 this.props.votingDetail.isVotingStart
          console.log('開啟投票畫面!');
          break;
        } else if (word == "議程") {
          //省略不做
    ...
    function OpenEvent(text_site) {
      for (let whichword = text_site; whichword <= (text_site + 8); whichword++) {
        let word = InputText.slice(whichword, whichword + 2);
        if (word == "電子白板" || word == "白板" || word == "畫板") {
          //開啟Meeting.js 的 this.props.isPaintOpen
          console.log('開啟電子白板!');
          break;
        } else if (word == "六頂") {
          //開啟MainScreen.js 的 this.props.isSixhatOpen
          console.log('開啟六頂思考帽遊戲!');
          break;
        } else if (word == "九宮" || word == "發散") {
          //開啟Meeting.js 的 this.props.isGridOpen
          console.log('開啟九宮格遊戲!');
          break;
        } else {
    ...
    function CopyEvent(text_site) {
      for (let whichword = text_site; whichword <= (text_site + 6); whichword++) {
        let word = InputText.slice(whichword, whichword + 2);
