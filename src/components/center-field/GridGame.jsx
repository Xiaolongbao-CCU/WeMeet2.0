...
class GridGame extends React.Component {
  ...
  onChangeInput(e) {
    ...
    const a = parseInt(key.substring(5, 6), 10);
    const b = parseInt(key.substring(7, 8), 10);
    const c = parseInt(key.substring(9, 10), 10);
    const todo = (array, value) => {
  ...
  render() {
    ...
    order.forEach((number, index) => {
      const name = `${'divTable '}${number}`;
      const innerGrid = [];
      for (let i = 0; i < 3; i += 1) {
        const outerKey = `${'grid:'}${index}:${i}`;
        const input = [];
        for (let j = 0; j < 3; j += 1) {
          const innerKey = `${outerKey}:${j}`;
          input.push(<div className="divTableCell">
            <textarea
        ...
        innerGrid.push(<div className="divTableRow" data={outerKey}>
          {input}
        </div>);
      }
